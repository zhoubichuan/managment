...
let distBasePath = 'managment/'; //构建输出的目录
let comPath = './'
let serve= require('./gulp/server.js')
//删除dist文件
gulp.task('task1-clean', function () {
    return gulp.src('managment', { allowEmpty: true }) //src 匹配文件
        .pipe(clean()); //清除dist目录
});
...
gulp.task('task4-js', function () {
    return gulp.src([comPath + '**/*.js','!gulpfile.js','!node_modules/**'])
        // .pipe(babel({
...
gulp.task('task5-image', function () {
    return gulp.src([comPath + 'images/*'])
        // .pipe(imgmin())
